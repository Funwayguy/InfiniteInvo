...
public class ConfigHandler
{
	public static void initConfigs(File file)
	{
		...
		II_Settings.unlockedSlots = config.getInt("No. Start Unlocked", Configuration.CATEGORY_GENERAL, 27, 0, Integer.MAX_VALUE - 100, "How many slots are initially unlocked (Recommend to keep at least 27 slots for)");
		II_Settings.unlockCost = config.getInt("Unlock cost", Configuration.CATEGORY_GENERAL, 100, 1, Integer.MAX_VALUE, "How many XP levels are required to unlock a slot");
		...
		II_Settings.unlockIncrease = config.getInt("Cost Increase", Configuration.CATEGORY_GENERAL, 10, 0, Integer.MAX_VALUE, "How much the unlock cost while increase per slot");
